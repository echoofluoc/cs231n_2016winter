...
def batchnorm_backward(dout, cache):
  ...
  dx_hat = dout*gamma
  dx_hat_num = dx_hat/(np.sqrt(var+eps))
  dx_hat_den = np.sum(dx_hat*(x-mean),axis=0)
  dxi = dx_hat_num
  dvar = -0.5*((var+eps)**(-1.5))*dx_hat_den
  # Note var is also a function of mean
  dmean = -np.sum(dx_hat_num,axis=0) + dvar*np.mean(-2*(x-mean),axis=0)
  dx_var = 2*(x-mean)*dvar/N
  dx_mean = dmean/N
  dx = dxi+dx_var+dx_mean
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def batchnorm_backward_alt(dout, cache):
  ...
  dgamma = np.sum(x_hat*dout,axis=0)
  dbeta = np.sum(dout,axis=0)
  dx = (gamma/np.sqrt(var+eps)/N)*(N*dout-np.sum(dout*(x-mean),axis=0)*(x-mean)/(np.sqrt(var+eps))**2 -np.sum(dout, axis=0))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    ###########################################################################
    stay = 1-p
    mask = (np.random.rand(*x.shape) < stay)/stay
    out = mask * x 
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  #############################################################################
  #N : the number of pictures
  N, C, H, W = x.shape
  F, C, HH, WW = w.shape
  stride = conv_param['stride']
  pad = conv_param['pad']
  #Compute the output matrix's shape
  ...
  for n in range(N):
        for f in range(F):
            ...
            for c in range(C):
                pedded_x = np.pad(x[n, c], pad_width=pad, mode='constant', constant_values=0)
...
def conv_backward_naive(dout, cache):
  ...
  #Note the pad function starts at col then row
        ...
        for f in range(F):
            for i in range(new_H):
                for j in range(new_W):
                    ...
                    padded_dx[n,:,i * stride: i * stride+HH, j * stride: j * stride + WW] += dout[n,f,i,j]*w[f]
...
def max_pool_forward_naive(x, pool_param):
  ...
  N,C,H,W = x.shape
  new_H = int((H-pool_H)/stride) + 1
  new_W = int((W-pool_W)/stride) + 1
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
        for c in range(C):
            for i in range(new_H):
                for j in range(new_W):
                    area = x[n,c,i*stride:i*stride+pool_H,j*stride:j*stride+pool_W]
                    dx[n,c,i*stride:i*stride+pool_H,j*stride:j*stride+pool_W] = (area==np.max(area))*dout[n,c,i,j]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
